...
int parse_properties(DBusMessageIter *iter, Properties *properties,
    ...
    array->num = 0;
    for (i = 0; i < max_num_properties; i++) {
		if(values[i].used) array->num ++;
    }
    if(array->num == 0) goto failure;
...
int parse_adapter_property_change( DBusMessage *msg,t_property_value_array *array){
    return parse_property_change(msg, (Properties *) &adapter_properties,
                    sizeof(adapter_properties) / sizeof(Properties),array);
}
...
int get_id_by_key(t_property_value_array *array, const char *name)
{
    int i;

	if (!array || !name) return -1;
    for(i = 0; i < array->num; i++) {
        if(strcmp(array->head[i].name, name) == 0) {
            return i;
        }
    }
    return -1;
...
int x_sdp_search(const bdaddr_t *src, const bdaddr_t *dst, uint16_t uuid, uint8_t *channel){
	sdp_session_t *session;
	sdp_list_t *search, *attributes, *next, *rsp =NULL, *protos =NULL;
       sdp_record_t *rec;
	uuid_t svclass;
	uint16_t attr;
       uint8_t ch = 0;
